...
LoadWeapons :: proc() {
	for cyc in 1..=no_weaps {
		if weapLocation[cyc] == gamLocation[slot] {
			...
			weapo_string := fmt.aprint("Weapons/{}.3ds", weapFile[weapType[cyc]])
		...
		if weapLocation[cyc] == gamLocation[slot] && weapState[cyc] > 0 {
			...
			if weapCarrier[cyc] == 0 {
				...
				if gamLocation[slot] == 2 && weapThrower[cyc] > 0 {
					...
					if weapX[cyc] > bb.EntityX(basket, 1) - 6 \
					&& weapX[cyc] < bb.EntityX(basket, 1) + 6 \
					&& weapZ[cyc] > bb.EntityZ(basket, 1) - 6 \
					&& weapZ[cyc] < bb.EntityZ(basket, 1) + 6 {
						if weapY[cyc] > bb.EntityY(basket, 1) - 8 \
						&& weapY[cyc] < bb.EntityY(basket, 1) - 2 \
						&& weapOldY[cyc] > bb.EntityY(basket, 1) - 2 {
							...
							charAgility[pChar[weapThrower[cyc]]] += bb.RndI(0, 1)
							charHappiness[pChar[weapThrower[cyc]]] += bb.RndI(1, 5)
							randy := bb.RndI(0, 5)
				...
				if weapFlight[cyc] > 0 && weapY[cyc] > bb.EntityY(weapGround[cyc]) + 0.5 {
					...
					&& weapZ[cyc] > bb.EntityZ(weapWall[cyc]) - 2.0 && weapZ[cyc] < bb.EntityZ(weapWall[cyc]) + 2.0 {
						if gotim > 0 do ProduceSound(weap[cyc], weapSound[weapType[cyc]], 22050, bb.RndF(0.1, 0.3))
						...
						if gamLocation[slot] == 2 {
							...
							&& weapZ[cyc] > bb.EntityZ(basket, 1) - 100 && weapZ[cyc] < bb.EntityZ(basket, 1) + 100 {
								weapFlightA[cyc] += bb.RndF(-90.0, 90.0)
							}
						}
						weapA[cyc] += bb.RndF(-20.0, 20.0)
						ExplodeWeapon(cyc, 0)
				...
				if weapFlight[cyc] > 0 && weapY[cyc] > bb.EntityY(weapGround[cyc]) + 0.5 {
					for v in 1..=no_plays {
						...
						if cast(bool)WeaponProximity(v, cyc, cast(i32)range) \
						&& weapY[cyc] >= pY[v] + 5 && weapY[cyc] <= bb.EntityY(bb.FindChild(p[v], "Head"), 1) + 5 \
						&& AttackViable(v) > 0 && weapSting[cyc][v] == 1 {
							...
							if blocked == 0 {
								...
								pHealth[v] -= bb.RndI(1, weapDamage[weapType[cyc]])
								pHP[v] -= bb.RndI(1, weapDamage[weapType[cyc]])
								...
								if AttackViable(v) >= 1 && AttackViable(v) <= 2 {
									...
									randy := bb.RndI(0, 3)
									if randy == 0 && weapY[cyc] >= pY[v] + 20 do pDazed[v] = bb.RndI(50, 200)
								}
								if AttackViable(v) == 3 {
									GroundReaction(v)
									pDT[v] = pDT[v] - 10
							...
							if weapStyle[weapType[cyc]] == 7 && blocked == 0 {
								...
								weapGravity[cyc] = 0
								weapFlight[cyc] = 0
								if weapName[weapType[cyc]] == "Syringe" && pInjured[v] < 100 do pInjured[v] = bb.RndI(100, 500)
							}
				...
				if weapFlight[cyc] > 0 {
					...
					if weapFlight[cyc] > 0.5 {
						...
						randy = bb.RndI(0, 60)
						if randy <= weapScar[cyc] && weapScar[cyc] > 1 do CreatePool(weapX[cyc], weapY[cyc], weapZ[cyc], bb.RndF(0.5, 2.0), 1, 1)
					}
				}
				bb.RotateEntity(weap[cyc], 0, weapA[cyc], 0)
				//gravity
...
FireBullet :: proc(cyc: i32) {
	...
	if v == 0 do v = bb.RndI(1, no_bullets)
	//initiate bullet
		...
		if bulletState[cyc] == 1 {
			for _ in 1..=10 {
				...
				if bulletState[cyc] == 1 {
					for count in 1..=bb.CountCollisions(bullet[cyc]) {
						if bb.CollisionEntity(bullet[cyc], count) > 0 {
							ProduceSound(bullet[cyc], sRicochet[bb.RndI(1, 3)], 22050, 0)
							ProduceSound(bullet[cyc], sImpact[bb.RndI(4, 5)], 22050, 0)
				...
				for v in 1..=no_plays {
					...
					if bulletX[cyc] > pX[v] - range && bulletX[cyc] < pX[v] + range \
					&& bulletZ[cyc] > pZ[v] - range && bulletZ[cyc] < pZ[v] + range \
					&& bulletY[cyc] >= pY[v] && bulletY[cyc] <= bb.EntityY(bb.FindChild(p[v], "Head"), 1) + 5 {
						if AttackViable(v) > 0 && bulletState[cyc] == 1 {
							...
							CreatePool(bulletX[cyc], pGround[v], bulletZ[cyc], bb.RndF(2.0, 8.0), 1, 1)
							if AttackViable(v) >= 1 && AttackViable(v) <= 2 {
...
ManageWeaponScars :: proc(cyc: i32) {
	if weapCarrier[cyc] == gamLocation[weapCarrier[cyc]] {
		// heal scars
		randy := bb.RndI(0, 10000 / gamSpeed[gamLocation[weapCarrier[cyc]]])
		if randy == 0 do weapScar[cyc] -= 1
		...
		if weapScar[cyc] != weapOldScar[cyc] {
			...
			if weapCarrier[cyc] > 0 {
				limb := bb.FindChild(p[weapCarrier[cyc]], weapFile[weapType[cyc]])
	...
	if no_weaps > 0 {
		for v in 1..=no_weaps {
			if weapLocation[v] == gamLocation[slot] {
				...
				if weapCarrier[v] > 0 do distance *= 2
				if weapY[v] < pY[cyc] - 30 || weapY[v] > pY[cyc] + 30 do distance *= 2
...
WeaponProximity :: proc(cyc, v, range: i32) -> i32 {
	value: i32 = 0
	if weapLocation[v] == gamLocation[slot] && weapState[v] == 1 \
	&& pY[cyc] > weapY[v] - 30 && pY[cyc] < weapY[v] + 30 {
		checkX := pX[cyc]
		checkZ := pZ[cyc]
		...
		if checkX > weapX[v]-cast(f32)range && checkX < weapX[v]+cast(f32)range \
		&& checkZ > weapZ[v]-cast(f32)range && checkZ < weapZ[v]+cast(f32)range {
			value = 1
		}
	}
	return value
...
WeaponImpact :: proc(cyc, v, blocked: i32) {
	...
	if w > 0 {
		...
		charReputation[pChar[cyc]] += bb.RndI(0, 1 + charRole[pChar[v]])
		...
		if blocked == 1 {
			...
			pHealth[v] -= bb.RndI(1.0, weapDamage[weapType[w]])
			if pAnim[cyc] != 211 {
				pHP[v] -= bb.RndI(1, weapDamage[weapType[w]])
			}
...
FindSmashes :: proc(cyc: i32) {
	...
	for v in 1..=no_weaps {
		...
		if weapLocation[v] == gamLocation[slot] && weapState[v] == 1 \
		&& cast(bool)WeaponProximity(cyc, v, cast(i32)range) \
		&& pY[cyc] > weapY[v]-5 && weapCarrier[v] == 0 {
			...
			if CountScars(cyc) >= 2 {
				ScarWeapon(v, 1)
				CreatePool(weapX[v], pGround[cyc], weapZ[v], bb.RndF(3.0, 10.0), 2, 1)
			}
			if weapStyle[weapType[v]] != 6 {
				bb.Animate(weap[v], 3, 2.0, 0, 1)
			}
			weapA[v] += bb.RndF(-30.0, 30.0)
			weapX[v] += bb.RndF(-2.0, 2.0)
			weapZ[v] += bb.RndF(-2.0, 2.0)
			pHealth[cyc] -= bb.RndI(1, weapDamage[weapType[v]])
...
ExplodeWeapon :: proc(cyc, chance: i32) {
	...
	if randy == 0 && weapStyle[weapType[cyc]] == 6 && (weapClip[cyc] > 0 || chance < 0) {
		switch weapType[cyc] {
		case 9: CreateExplosion(weapThrower[cyc], weap[cyc], weapX[cyc], weapY[cyc], weapZ[cyc], 10)
		case 19: CreateExplosion(weapThrower[cyc], weap[cyc], weapX[cyc], weapY[cyc], weapZ[cyc], 11)
		case 18: CreateExplosion(weapThrower[cyc], weap[cyc], weapX[cyc], weapY[cyc], weapZ[cyc], 13)
		}
		weapState[cyc] = 0
